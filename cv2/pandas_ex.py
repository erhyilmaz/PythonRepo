...
df = pd.DataFrame(data=data, index=columns).T  # order of keys in dict is guaranteed
...
# New entry to append
new_entry = {'TUR': {'COUNTRY': 'Turkey', 'POP': 80.53, 'AREA': 2_724.90, 'GDP': 59.41, 'CONT': 'Europe', 'IND_DAY': '193-10-29'}}
df_new = pd.DataFrame(data=new_entry, index=columns).T  # order of keys in dict is guaranteed
